...
class SupabaseManager:
    ...
    async def get_events_since(self, city_id: str, since: str) -> List[Dict]:
        """
    ...
    async def get_city_events(self, city_id: str, limit: int = 100) -> List[Dict]:
        """
...
async def sync_events_to_supabase(events_data: List[Dict], city_id: str) -> Dict:
    """
    Wrapper function to sync events to Supabase.
    Should be called after local database save.
    """
    return await supabase_manager.sync_events(events_data, city_id)


async def get_recent_events_from_supabase(city_id: str, minutes: int = 5) -> List[Dict]:
    """
    ...
    return await supabase_manager.get_events_since(city_id, since)
